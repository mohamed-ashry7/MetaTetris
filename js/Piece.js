...
const rotated = (piece, cclk=false )=>{
 ...
 const isOccupied = (piece,anchor,board)=>{
    for(const [i,j] of piece){
        ...
        if (x<0 || x>= board.shape[0] || y>= board.shape[1] || board.getCoordinates(x,y)){
            return true;
        }
 ...
 const left = (piece,anchor, board)=>{
    ...
    return isOccupied(piece,newAnchor,board)?[piece,anchor]:[piece,newAnchor]
};
 ...
 const rotateLeft = (piece,anchor,board)=>{
    ...
    return isOccupied(newPiece,anchor,board)?[piece,anchor]:[newPiece,anchor]
};
...
module.exports= {
    isOccupied,newPiece,rotated,rotateLeft,rotateRight,left,right,softDrop,hardDrop,idle
}